...
# Ensure we can import the backend package without modifying backend files (avoid reload)
sys.path.insert(0, 'd:/CRM-postgres/backend')
...
try:
    ...
    if 'Azure' in svc_type or 'Blob' in svc_type or hasattr(storage, 'blob_service_client'):
        ...
        base_path = getattr(storage, 'base_path', None) or getattr(
            storage, 'upload_dir', None) or getattr(storage, 'uploads_dir', None)
