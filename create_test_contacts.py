...
def create_test_contacts():
    ...
    try:
        ...
        company_response = requests.post("http://localhost:8000/api/v1/companies", json=company_data, headers=headers)
        ...
        if company_response.status_code == 200:
            ...
            print(f"Created company: {company_result.get('name')} with ID: {company_id}")
        ...
        company_response2 = requests.post("http://localhost:8000/api/v1/companies", json=company_data2, headers=headers)
        company_id2 = None
        if company_response2.status_code == 200:
            company_result2 = company_response2.json()
            company_id2 = company_result2.get('id')
            print(f"Created company: {company_result2.get('name')} with ID: {company_id2}")
        ...
        for contact_data in contacts_data:
            contact_response = requests.post("http://localhost:8000/api/v1/contacts", json=contact_data, headers=headers)
            print(f"Contact creation status: {contact_response.status_code}")
            
            if contact_response.status_code == 200:
                contact_result = contact_response.json()
                print(f"Created contact: {contact_result.get('first_name')} {contact_result.get('last_name')} with ID: {contact_result.get('id')}")
            else:
