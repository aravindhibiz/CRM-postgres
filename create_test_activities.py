    ...
    try:
        ...
        if len(contacts) >= 2:
            ...
            for activity_data in activities_data:
                activity_response = requests.post("http://localhost:8000/api/v1/activities", json=activity_data, headers=headers)
                print(f"Activity creation status: {activity_response.status_code}")
                
                if activity_response.status_code == 200:
                    activity_result = activity_response.json()
                    print(f"Created activity: {activity_result.get('subject')} with ID: {activity_result.get('id')}")
                else:
                    print(f"Activity creation failed: {activity_response.text}")
