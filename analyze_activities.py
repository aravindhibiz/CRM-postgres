...
def analyze_activities_and_contacts():
    ...
    try:
        ...
        activities_response = requests.get(
            "http://localhost:8000/api/v1/activities", headers=headers)
        if activities_response.status_code == 200:
            activities = activities_response.json()
            print(f"Number of activities: {len(activities)}")
            for i, activity in enumerate(activities):
                print(
                    f"{i+1}. Activity: {activity.get('subject')} | Type: {activity.get('type')} | Contact ID: {activity.get('contact_id')}")
        else:
        ...
        if contacts_response.status_code == 200:
            ...
            for i, contact in enumerate(contacts):
                ...
                company_name = contact.get('company', {}).get(
                    'name', 'No Company') if contact.get('company') else 'No Company'
                print(
                    f"{i+1}. Contact: {contact_name} | ID: {contact.get('id')} | Company: {company_name}")
        else:
        ...
        if activities_response.status_code == 200 and contacts_response.status_code == 200:
            ...
            for contact in contacts:
                contact_map[contact.get(
                    'id')] = f"{contact.get('first_name')} {contact.get('last_name')}"

            print("Activities and their associated contacts:")
            for activity in activities:
                contact_id = activity.get('contact_id')
                contact_name = contact_map.get(
                    contact_id, 'UNLINKED - No contact found')
                print(
                    f"- '{activity.get('subject')}' -> Contact: {contact_name} (ID: {contact_id})")
